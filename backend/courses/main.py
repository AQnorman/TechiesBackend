...
router = _fastapi.APIRouter(prefix="/api/courses", tags=["Courses"])

# dependencies=[_fastapi.Depends(JWTBearer())]


@router.post("/", response_model=_schemas.Course)
async def create_course(course: _schemas.CourseCreate, instructor_id: int, db: _orm.Session = _fastapi.Depends(get_db)):
    return await _services.create_course(db=db, course=course, instructor_id=instructor_id)
